/**
 * The starter dataset, translated for Postgres.
 *
 * It reuses `buildSeed()` — the same generator the in-memory provider uses —
 * so there is one definition of the demo data rather than two that drift. The
 * only work here is the translation: ISO strings become Date objects, typed
 * objects become Json, and rows are inserted parents-first to satisfy foreign
 * keys.
 */
...
/**
 * Loads the starter dataset. Every table is cleared first, so this is only
 * ever called against a database that is known to be empty or one the caller
 * has deliberately chosen to reset.
 */
