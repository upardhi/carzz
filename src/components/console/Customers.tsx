  ...
  const filtered = all.filter((customer) => {
    ...
    if (query || rawQuery) {
      ...
      const haystack = [
        ...
        user?.name ?? '',
        user?.phone ?? '',
        ...own.flatMap((c) => [c.make, c.model, c.plate, c.colour]),
      ]
        .filter(Boolean)
        .join(' ')
        .toLowerCase();
        ...
        columns={[
          {
            ...
            render: (customer) => {
              ...
              const user = (customer.userId ? userByUserId.get(customer.userId) : null) ?? userByCustomerId.get(customer.id);
              const areaName = areaById.get(customer.areaId)?.name ?? '—';
                ...
                <div className="flex items-center gap-2.5">
                  ...
                  <div className="min-w-0">
                    <Link
                      ...
                      className="block font-semibold text-slate-900 text-xs sm:text-sm hover:text-blue-600 transition-colors truncate"
                    >
                      {customer.name}
                    </Link>
                    <div className="flex items-center gap-1.5 text-[11px] font-medium text-slate-400 truncate">
                      ...
                      {user?.email ? (
                        <>
                          ...
                          <span className="text-slate-500 font-normal truncate">{user.email}</span>
                        </>
                      ) : null}
                    </div>
                  </div>
