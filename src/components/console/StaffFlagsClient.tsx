    ...
    },
    {
      ...
      render: (item) => (
        <div className="text-xs">
          <span className={`font-semibold ${item.offs > item.allowedOffs ? 'text-danger-600' : 'text-ink'}`}>
            {item.offs}
          </span>
    ...
    },
    {
      ...
      render: (item) => {
        if (item.isWarningOnly && item.uninformed === 0) {
          return (
            <span className="inline-flex items-center gap-1 rounded px-2 py-0.5 text-xs font-semibold text-warning-700 bg-warning-50 border border-warning-200">
              Warning
            </span>
          );
        }
        return (
          <span className="inline-flex items-center gap-1 rounded px-2 py-0.5 text-xs font-semibold text-danger-600 bg-danger-50 border border-danger-100">
            Deduction Pending
          </span>
        );
      },
    },
    {
      ...
      render: (item) => {
        if (item.totalPenalty === 0) {
          return <span className="text-xs font-semibold text-warning-700">₹0 (Warning)</span>;
        }
        return <span className="text-danger-600 font-semibold text-sm">−{money(item.totalPenalty)}</span>;
      },
    },
  ];
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-start gap-3 rounded-2xl border border-warning-200 bg-warning-50 p-4 shadow-2xs">
        ...
        <div className="text-xs text-warning-900 leading-relaxed">
          <span className="font-semibold">Automated Attendance Tracking: </span>
