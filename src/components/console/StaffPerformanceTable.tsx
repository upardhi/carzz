...
export function StaffPerformanceTable({ rows, areas, totalStaff, cycle }: Props) {
  ...
  const totalFlagged = useMemo(() => rows.reduce((s, r) => s + r.flaggedWashes, 0), [rows]);
  ...
  }, [rows, debouncedSearch, selectedArea, areaMap]);
    ...
    return [...filtered].sort((a, b) => {
      ...
      switch (sortColumn) {
        ...
        case 'complaints':
          ...
          break;
        case 'flagged':
          valA = a.flaggedWashes;
          valB = b.flaggedWashes;
          break;
        default:
          valA = a.washes;
          valB = b.washes;
  ...
  const columns: DataTableColumn<StaffPerformanceRow>[] = [
    ...
    },
    {
      ...
      align: 'center',
      render: (row) =>
        row.averageManagerRating ? `${row.averageManagerRating.toFixed(1)} ★` : '—',
    },
  ];
