...
export function PocketRequestsClient({ base, backHref }: PocketRequestsClientProps) {
  ...
  const columns: Column<PocketRequestItem>[] = [
      ...
      render: (item, idx) => {
        ...
        return (
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <div className="font-semibold text-ink text-sm">{item.staffName}</div>
      ...
      render: (item) => (
        <div className="text-xs">
          <div className="font-medium text-ink">
            Limit: <span className="font-semibold">{money(item.availableAllowance)}</span>
          </div>
    ...
    },
    {
      ...
      render: (item) =>
        item.overCap ? (
          <div>
            <span className="rounded px-2 py-0.5 text-[10px] font-semibold text-danger-600 bg-danger-50 border border-danger-100">
              Over limit
            </span>
          ...
          <span className="rounded px-2 py-0.5 text-[10px] font-semibold text-success-600 bg-success-50 border border-success-100">
            Within limit
          </span>
        ),
    },
    {
      ...
      render: (item) => {
        if (item.status === 'PENDING') {
          return (
            <span className="inline-flex items-center gap-1 rounded-full bg-warning-50 border border-warning-200 px-2.5 py-0.5 text-xs font-semibold text-warning-700">
        ...
        if (item.status === 'APPROVED' || item.status === 'PAID') {
          return (
            <span className="inline-flex items-center gap-1 rounded-full bg-success-50 border border-success-200 px-2.5 py-0.5 text-xs font-semibold text-success-700">
        ...
        return (
          <span className="inline-flex items-center gap-1 rounded-full bg-surface-muted border border-line px-2.5 py-0.5 text-xs font-semibold text-ink-mute">
        ...
        return (
          <div className="flex items-center gap-2">
            <button
              ...
              className="rounded-lg bg-success-600 px-3 py-1.5 text-xs font-semibold text-white shadow-2xs hover:bg-success-700 disabled:opacity-50 transition-colors"
            >
            ...
            <button
              ...
              className="rounded-lg bg-danger-600 px-3 py-1.5 text-xs font-semibold text-white shadow-2xs hover:bg-danger-700 disabled:opacity-50 transition-colors"
            >
              Reject
            </button>
          </div>
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        ...
        <button
          type="button"
          onClick={() => fetchData()}
          className="inline-flex items-center gap-1.5 rounded-xl border border-line bg-surface px-3.5 py-2 text-xs font-semibold text-ink shadow-2xs hover:bg-surface-muted transition-colors"
        >
