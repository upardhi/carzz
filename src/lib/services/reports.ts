...
/**
 * Drops every cached area-performance entry, across every cycle and filter.
 *
 * Call this whenever an area row itself changes — created, edited, disabled,
 * or deleted — since the cache embeds the raw `Area` row (including `active`)
 * inside each result and otherwise serves it stale for up to 60s.
 */
export function invalidateAreaPerformanceCache(): void {
  areaPerfCache.clear();
}
...
async function _computeStaffPerformance(
  ...
  const [totalCount, staff, settings] = await Promise.all([
    store.staff.count({
      role: 'EMPLOYEE',
      ...areaFilter,
    } as never),
  ...
  const rows: StaffPerformanceRow[] = staff.map((s) => {
    ...
    const done = visits.filter((v) => v.status === 'DONE');
    const rated = done.filter((v) => v.rating !== null);
    const managerRated = done.filter((v) => v.managerRating !== null);
    const flaggedWashes = done.filter(
      (v) => washSpeedFlag(washDurationMinutes(v), settings) !== null,
    ).length;
    ...
    return {
      ...
      flaggedWashes,
      averageManagerRating: managerRated.length
        ? managerRated.reduce((sum, v) => sum + (v.managerRating ?? 0), 0) / managerRated.length
        : 0,
    };
  });
