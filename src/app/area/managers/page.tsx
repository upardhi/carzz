...
export default async function AreaAdminManagers() {
  ...
  const [managers, performance, users, areas] = await Promise.all([
    store.staff.find({
      where: {
        role: 'MANAGER',
  ...
  const canAddManager =
    session.user.role === 'SUPER_ADMIN' || can(session.user.role, 'user:manage') || can(session.user.role, 'staff:create');
